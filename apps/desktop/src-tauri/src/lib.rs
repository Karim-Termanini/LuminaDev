...
fn write_json(path: &PathBuf, value: &Value) -> Result<(), String> {
    let content =
        serde_json::to_string_pretty(value).map_err(|e| format!("[STORE_ENCODE_ERROR] {}", e))?;
...
async fn ipc_invoke(
    ...
    let res = match channel.as_str() {
        "dh:session:info" => {
            json!({ "ok": true, "mode": "tauri", "platform": std::env::consts::OS })
        }
        "dh:store:get" => {
        ...
        "dh:host:distro" => {
            let distro = exec_output(
                ...
                &[
                    "-lc",
                    "source /etc/os-release >/dev/null 2>&1; echo ${ID:-linux}",
                ],
            )
            .await
        ...
        "dh:docker:check-installed" => {
            let docker = exec_output(
                "bash",
                &["-lc", "command -v docker >/dev/null 2>&1 && echo ok"],
            )
            .await
            .is_ok();
            let compose = exec_output(
                "bash",
                &["-lc", "docker compose version >/dev/null 2>&1 && echo ok"],
            )
            .await
            .is_ok();
            let buildx = exec_output(
                "bash",
                &["-lc", "docker buildx version >/dev/null 2>&1 && echo ok"],
            )
            .await
            .is_ok();
            json!({ "docker": docker, "compose": compose, "buildx": buildx })
        }
        "dh:docker:list" => match exec_output("docker", &["ps", "-a", "--format", "{{json .}}"])
            .await
        {
            Ok(out) => {
        ...
        "dh:docker:logs" => {
            ...
            if id.is_empty() {
                ...
                match exec_result("docker", &["logs", "--tail", &tail, id]).await {
                    Ok((stdout, stderr)) => {
                        json!({ "ok": true, "log": format!("{}{}", stdout, stderr) })
                    }
                    Err(e) => {
                        json!({ "ok": false, "log": "", "error": format!("[DOCKER_LOGS_FAILED] {}", e.trim()) })
                    }
                }
            }
        }
        "dh:docker:images:list" => match exec_output(
            "docker",
        ...
        .await
        {
            ...
            Err(e) => {
                json!({ "ok": false, "error": format!("[DOCKER_IMAGES_FAILED] {}", e.trim()) })
            }
        },
        ...
        "dh:docker:volumes:list" => {
          ...
          .collect();
                ...
                Err(e) => {
                    json!({ "ok": false, "error": format!("[DOCKER_VOLUMES_FAILED] {}", e.trim()) })
                }
            }
        }
        "dh:docker:volume:create" => {
            ...
            if name.is_empty() {
                ...
                match exec_output("docker", &["volume", "create", name]).await {
                    Ok(_) => json!({ "ok": true }),
                    Err(e) => {
                        json!({ "ok": false, "error": format!("[DOCKER_VOLUME_CREATE_FAILED] {}", e.trim()) })
                    }
                }
            }
        }
        "dh:docker:volume:action" => {
            ...
            if name.is_empty() || action != "remove" {
                ...
                match exec_output("docker", &["volume", "rm", name]).await {
                    Ok(_) => json!({ "ok": true }),
                    Err(e) => {
                        json!({ "ok": false, "error": format!("[DOCKER_VOLUME_ACTION_FAILED] {}", e.trim()) })
                    }
                }
            }
        }
        "dh:docker:networks:list" => {
          ...
          .collect();
                ...
                Err(e) => {
                    json!({ "ok": false, "error": format!("[DOCKER_NETWORKS_FAILED] {}", e.trim()) })
                }
            }
        }
        "dh:docker:network:create" => {
            ...
            if name.is_empty() {
                ...
                match exec_output("docker", &["network", "create", name]).await {
                    Ok(_) => json!({ "ok": true }),
                    Err(e) => {
                        json!({ "ok": false, "error": format!("[DOCKER_NETWORK_CREATE_FAILED] {}", e.trim()) })
                    }
                }
            }
        }
        "dh:docker:network:action" => {
            ...
            if id.is_empty() || action != "remove" {
                ...
                match exec_output("docker", &["network", "rm", id]).await {
                    Ok(_) => json!({ "ok": true }),
                    Err(e) => {
                        json!({ "ok": false, "error": format!("[DOCKER_NETWORK_ACTION_FAILED] {}", e.trim()) })
                    }
                }
            }
        }
        "dh:docker:prune" => {
            match exec_output("docker", &["system", "prune", "-f", "--volumes"]).await {
                ...
                Err(e) => {
                    json!({ "ok": false, "error": format!("[DOCKER_PRUNE_FAILED] {}", e.trim()) })
                }
            }
        }
        "dh:docker:prune:preview" => {
            let containers = exec_output(
                "bash",
                &["-lc", "docker ps -a -q --filter status=exited | wc -l"],
            )
            .await
            .unwrap_or_else(|_| "0".to_string());
            let images = exec_output(
                "bash",
                &["-lc", "docker images -f dangling=true -q | wc -l"],
            )
            .await
            .unwrap_or_else(|_| "0".to_string());
            let volumes = exec_output(
                "bash",
                &["-lc", "docker volume ls -qf dangling=true | wc -l"],
            )
            .await
            .unwrap_or_else(|_| "0".to_string());
            let networks = exec_output(
                "bash",
                &["-lc", "docker network ls -qf dangling=true | wc -l"],
            )
            .await
            .unwrap_or_else(|_| "0".to_string());
            json!({
        ...
        "dh:docker:cleanup:run" => {
            ...
            {
                logs.push(
                    exec_output("docker", &["container", "prune", "-f"])
                        .await
                        .unwrap_or_else(|e| e),
                );
            }
            ...
            {
                logs.push(
                    exec_output("docker", &["image", "prune", "-af"])
                        .await
                        .unwrap_or_else(|e| e),
                );
            }
            ...
            {
                logs.push(
                    exec_output("docker", &["volume", "prune", "-f"])
                        .await
                        .unwrap_or_else(|e| e),
                );
            }
            ...
            {
                logs.push(
                    exec_output("docker", &["network", "prune", "-f"])
                        .await
                        .unwrap_or_else(|e| e),
                );
            }
        ...
        "dh:docker:pull" => {
            ...
            if image.is_empty() {
                ...
                match exec_result("docker", &["pull", image]).await {
                    ...
                    Err(e) => {
                        json!({ "ok": false, "error": format!("[DOCKER_PULL_FAILED] {}", e.trim()) })
                    }
                }
            }
        }
        "dh:docker:search" => {
            ...
            .await
            {
              ...
              .collect();
                    ...
                    Err(_) => {
                        json!({ "ok": false, "error": "[DOCKER_SEARCH_FAILED] Invalid response format." })
                    }
                },
                Err(e) => {
                    json!({ "ok": false, "error": format!("[DOCKER_SEARCH_FAILED] {}", e.trim()) })
                }
            }
        }
        "dh:docker:tags" => {
            ...
            let url = format!(
                "https://hub.docker.com/v2/repositories/{}/{}/tags/?page_size=20",
                namespace, repo
            );
            match exec_output("curl", &["-fsSL", &url]).await {
                Ok(raw) => match serde_json::from_str::<Value>(&raw) {
                    ...
                    Err(_) => {
                        json!({ "ok": false, "error": "[DOCKER_TAGS_FAILED] Invalid response format." })
                    }
                },
                Err(e) => {
                    json!({ "ok": false, "error": format!("[DOCKER_TAGS_FAILED] {}", e.trim()) })
                }
            }
        }
        "dh:compose:up" | "dh:compose:logs" => {
            ...
            let cmd = if channel == "dh:compose:up" {
                ...
                format!(
                    "cd '{}' && docker compose logs --tail 200",
                    dir.replace('\'', "'\\''")
                )
            };
            match exec_result("bash", &["-lc", &cmd]).await {
                ...
                Err(e) => {
                    json!({ "ok": false, "log": "", "error": format!("[DOCKER_COMPOSE_FAILED] {}", e.trim()) })
                }
            }
        }
        ...
        "dh:terminal:create" => {
                ...
                .spawn()
            {
                ...
                Err(e) => {
                    json!({ "ok": false, "error": format!("[TERMINAL_CREATE_FAILED] {}", e) })
                }
            }
        }
        "dh:docker:terminal" => {
            ...
            if container_id.is_empty() {
                    ...
                    .spawn()
                {
                    ...
                    Err(e) => {
                        json!({ "ok": false, "error": format!("[DOCKER_TERMINAL_FAILED] {}", e) })
                    }
                }
            }
        }
        ...
        "dh:git:config:set" => {
            ...
            if ok {
                if let Some(branch) = default_branch {
                    if let Err(e) =
                        exec_output("git", &["config", "--global", "init.defaultBranch", branch])
                            .await
                    {
                        ok = false;
                        err = e;
            ...
            if ok {
                if let Some(editor) = default_editor {
                    if let Err(e) =
                        exec_output("git", &["config", "--global", "core.editor", editor]).await
                    {
                        ok = false;
                        err = e;
                    }
        ...
        "dh:git:clone" => {
            ...
            if url.is_empty() || target_dir.is_empty() {
                ...
                match exec_output("git", &["clone", url, target_dir]).await {
                    Ok(_) => json!({ "ok": true }),
                    Err(e) => {
                        json!({ "ok": false, "error": format!("[GIT_CLONE_FAILED] {}", e.trim()) })
                    }
                }
            }
        }
        ...
        "dh:ssh:get:pub" => {
            ...
            match std::fs::read_to_string(&pub_path) {
                ...
                Err(_) => {
                    json!({ "ok": false, "pub": "", "fingerprint": "", "error": "[SSH_KEY_NOT_FOUND] Missing public key." })
                }
            }
        }
        ...
        "dh:host:sysinfo" => {
            ...
            let os_name = exec_output("uname", &["-s"])
                .await
                .unwrap_or_else(|_| "Linux".to_string());
            let uptime = exec_output(
                "bash",
                &["-lc", "cut -d. -f1 /proc/uptime 2>/dev/null || echo 0"],
            )
            .await
            .unwrap_or_else(|_| "0".to_string())
            .trim()
        ...
        | "dh:docker:install" => invoke_node_bridge(&channel, &body).await,
