  ...
  let res = match channel.as_str() {
    ...
    "dh:terminal:create" => {
      ...
      let args: Vec<String> = body
        ...
        .unwrap_or_else(|| vec![
          "-c".to_string(),
          "if ! command -v script >/dev/null 2>&1; then echo '[TERMINAL_UNAVAILABLE] util-linux script is required for interactive terminal mode.'; exit 1; fi; exec script -qfec \"if command -v bash >/dev/null 2>&1; then exec bash --noprofile --norc -i; else exec sh -i; fi\" /dev/null".to_string(),
        ]);
      let _ = (cols, rows);
    ...
    "dh:docker:terminal" => {
      ...
      if container_id.is_empty() {
        ...
        let run_cmd = format!(
          "if ! command -v script >/dev/null 2>&1; then echo '[DOCKER_TERMINAL_FAILED] util-linux script is required for interactive terminal mode.'; exit 1; fi; exec script -qfec \"docker exec -it {} sh\" /dev/null",
          container_id
        );
