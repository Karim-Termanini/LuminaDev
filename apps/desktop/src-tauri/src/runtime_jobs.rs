    ...
    if in_flatpak && kind != "runtime_uninstall" && runtime_id != "rust" {
        ...
        let result: Result<(), String> = match kind.as_str() {
            ...
            v = v
                    ...
                    let mut v =
                        lumina_first_version_token(&version).unwrap_or_else(|| "0.13.0".into());
            ...
            v = v
             ...
             && pyenv global {v}",
                ...
                } else if runtime_id == "dotnet"
                    && (method.trim() == "local" || pkg_mgr == "pacman")
                {
  ...
  echo "[RUNTIME_INSTALL_FAILED] No supported package manager found" >&2; exit 1
                ...
                if runtime_id == "rust" {
                    ...
                    .await
                    {
                        Ok(out) => {
                            ...
                            {
                                logs.push(
                                    "already latest — rustup reports no updates.".to_string(),
                                );
                            } else {
            ...
            "profile_switch" => {
                ...
                if !to_dir.is_dir() {
                    ...
                    if !from_profile.is_empty() && from_profile != "none" {
                        let from_dir =
                            compose_profiles::compose_profile_workdir(&app, &from_profile);
                        if from_dir.is_dir() {
        ...
        if let Err(e) = result {
            ...
            if matches!(kind.as_str(), "runtime_install" | "install_deps") {
                runtime_append_verify(&runtime_id, &effective_verify_method, &version, &mut logs)
                    .await;
            }
        }
    }
