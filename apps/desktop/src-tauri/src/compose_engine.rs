...
async fn exec_docker_compose_in_dir(
    ...
    let compose_args =
        build_compose_args(compose_dir, compose_subargs, project_name, use_full_overlay);
    let fut = async {
...
pub(crate) async fn exec_docker_compose_up_streaming(
    ...
    let compose_args = build_compose_args(
        compose_dir,
        &["up", "-d", "--force-recreate"],
        Some(project_name),
        use_full_overlay,
    ...
    let mut child = cmd.spawn().map_err(|e| format!("[EXEC_ERROR] {}", e))?;
...
#[cfg(test)]
mod tests {
    use super::compose_up_progress_from_line;
