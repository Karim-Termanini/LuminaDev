...
pub(crate) async fn docker_check_installed() -> Value {
    let docker = exec_output_limit("docker", &["--version"], cmd_timeout_short())
        .await
        .is_ok();
    let compose = exec_output_limit("docker", &["compose", "version"], cmd_timeout_short())
        .await
        .is_ok();
    let buildx = exec_output_limit("docker", &["buildx", "version"], cmd_timeout_short())
        .await
        .is_ok();
...
pub(crate) async fn docker_action(body: &Value) -> Value {
    ...
    if action == "remove" {
        let remove_volumes = body
            .get("removeVolumes")
            .and_then(|v| v.as_bool())
            .unwrap_or(false);
        let remove_image = body
            .get("removeImage")
            .and_then(|v| v.as_bool())
            .unwrap_or(false);
        let image_ref = body
...
pub(crate) async fn docker_prune_preview(_body: &Value) -> Value {
    let containers =
        docker_nonempty_line_count(&["ps", "-a", "-q", "--filter", "status=exited"]).await;
