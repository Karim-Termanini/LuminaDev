...
use crate::host_exec::{cmd_timeout_short, exec_output_limit, get_global_ipc_timeout};
...
pub(crate) async fn profile_switch(app: &AppHandle, body: &Value) -> Value {
  ...
  match exec_output_limit("docker", &["info", "--format", "{{.ServerVersion}}"], cmd_timeout_short()).await {
    Err(_) => return json!({ "ok": false, "log": "", "error": "[DOCKER_UNAVAILABLE] Docker daemon is not reachable" }),
    Ok(ref out) if out.trim().is_empty() => return json!({ "ok": false, "log": "", "error": "[DOCKER_UNAVAILABLE] Docker daemon is not reachable" }),
    Ok(_) => {}
  }
  ...
  if let Some(from) = from_profile {
    ...
    if from_dir.is_dir() {
      match crate::compose_engine::expose_exec_docker_compose_in_dir(&from_dir, &["stop"], get_global_ipc_timeout(), Some(from), Some(get_profile_extra_env(app, from))).await {
  ...
  match crate::compose_engine::expose_exec_docker_compose_in_dir(&to_dir, &["up", "-d"], get_global_ipc_timeout(), Some(to_profile), Some(get_profile_extra_env(app, to_profile))).await {
    Ok((stdout, stderr)) => {
