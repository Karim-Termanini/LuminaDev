...
/// Async wrappers — use these in Tauri command handlers to avoid blocking the async runtime.
pub async fn read_json_async(path: PathBuf) -> Value {
    tokio::task::spawn_blocking(move || read_json(&path))
        .await
        .unwrap_or_else(|_| json!({}))
}

pub async fn write_json_async(path: PathBuf, value: Value) -> Result<(), String> {
    tokio::task::spawn_blocking(move || write_json(&path, &value))
        .await
        .map_err(|e| format!("[STORE_WRITE_ERROR] {}", e))?
}
...
pub fn docker_prune_preview_payload(
    containers: u64,
    images: u64,
    volumes: u64,
    networks: u64,
