...
describe('humanizeDockerError', () => {
  it('maps stable docker error prefixes to user-safe messages', () => {
    expect(humanizeDockerError('[DOCKER_UNAVAILABLE] cannot connect to docker daemon')).toContain(
      'Docker daemon/socket unavailable'
    )
    expect(humanizeDockerError('[DOCKER_PERMISSION_DENIED] permission denied')).toContain(
