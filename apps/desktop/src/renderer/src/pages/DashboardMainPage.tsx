...
// Slow ticker: while switch active and progress stuck between 60-95, nudge +0.4% every 800ms
  ...
  function handleConfirmSwitch(): void {
    ...
    _swSet({ active: true, step: t('main.switch.step.starting'), progress: 0, targetProfile: selectedProfileName ?? '' })
    const isRestart = activeProfile === selectedProfileName
    setToast({ type: 'success', message: isRestart ? t('main.toast.restarting', { name: selectedProfileName }) : t('main.toast.switching', { name: selectedProfileName }) })
    ...
    window.dh.profileSwitch({ from: (activeProfile as ComposeProfile) ?? undefined, to: selectedProfileName as ComposeProfile }).then((r) => {
      ...
      if (r.ok) {
        window.dh.storeSet({ key: 'active_profile', data: selectedProfileName }).then(() => {
          ...
          setToast({ type: 'success', message: t('main.toast.switched', { name: selectedProfileName }) })
          void refresh()
          ...
          <>
            ...
            {swState.active && swState.targetProfile === selectedProfileName && (
              <div style={{ marginTop: 20, padding: '16px 20px', borderRadius: 10, background: 'rgba(0,0,0,0.35)', border: `1px solid ${selectedProfile.accent}44` }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                   <span style={{ fontSize: 13, fontWeight: 600, color: selectedProfile.accent }}>{swState.step || t('main.switch.step.starting')}</span>
            ...
            {activeProfile === selectedProfileName && (
              <div style={{ marginTop: 24, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  ...
                  { label: t('main.health.env'), status: projectPath ? 'ok' as const : 'unknown' as const },
              ...
              <div style={{ marginTop: 32 }}>
                <div className="dashboard-widget">
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
                    <DashboardMetricBar label={t('main.metrics.cpu')} valueText={`${m.cpuUsagePercent.toFixed(0)}%`} percent={m.cpuUsagePercent} />
                    <DashboardMetricBar label={t('main.metrics.ram')} valueText={`${((m.totalMemMb - m.freeMemMb) / 1024).toFixed(1)} / ${(m.totalMemMb / 1024).toFixed(1)} GB`} percent={ramUsedPct} />
               ...
               <div style={{ marginTop: 32 }}>
                 <div className="dashboard-widget">
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                      <div>
                         <label style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 8, display: 'block' }}>{t('main.workspace.projectPath')}</label>
                        <div style={{ display: 'flex', gap: 12 }}>
                          ...
                          {!projectPath ? (
                            <>
                              <button onClick={handleLinkProject} style={{ padding: '0 16px', borderRadius: 6, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', color: 'var(--text)', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>{t('main.workspace.linkExisting')}</button>
                              <button onClick={() => {
                              ...
                              }} style={{ padding: '0 16px', borderRadius: 6, border: 'none', background: selectedProfile.accent, color: '#fff', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>{t('main.workspace.createNew')}</button>
                            </>
                          ) : (
                            <button onClick={async () => {
                               setProjectPath(null);
                               await window.dh.storeSet({ key: `project_dir_${selectedProfileName}`, data: null } as any);
                                setToast({ type: 'success', message: t('main.toast.projectUnlinked') });
                             }} style={{ padding: '0 16px', borderRadius: 6, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,0,0,0.1)', color: 'var(--text)', cursor: 'pointer', fontSize: 13, fontWeight: 600, transition: 'background 0.2s' }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,0,0,0.2)'} onMouseLeave={e => e.currentTarget.style.background = 'rgba(255,0,0,0.1)'}>{t('main.workspace.unlinkProject')}</button>
                          )}
                        </div>
                     </div>
                     {projectPath && (
                       <div>
                          <label style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 8, display: 'block' }}>{t('main.workspace.openInEditor')}</label>
                         <div style={{ display: 'flex', gap: 12 }}>
                           <select 
                           ...
                           >
                               {installedEditors.length === 0 && <option value="">{t('main.workspace.noEditors')}</option>}
                              {installedEditors.map(ed => (
                                 <option key={ed.name} value={ed.cmd}>{ed.name}</option>
                              ))}
                           </select>
                            <button onClick={handleOpenEditor} disabled={!selectedEditorCmd} style={{ padding: '0 16px', borderRadius: 6, border: 'none', background: 'var(--green)', color: '#fff', cursor: selectedEditorCmd ? 'pointer' : 'not-allowed', fontSize: 13, fontWeight: 600 }}>{t('main.workspace.openIDE')}</button>
                         </div>
                       </div>
                     )}
                   </div>
                    ...
                    <>
                      <div style={{ display: 'flex', gap: 16, marginBottom: 16, flexWrap: 'wrap' }}>
                        ...
                        <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: 'var(--text-muted)' }}>
                          ...
                          {gitStatus.staged.length + gitStatus.unstaged.length} {t('main.git.changed')}
                          {gitStatus.staged.length > 0 && <span style={{ color: 'var(--green)', fontSize: 11 }}>({gitStatus.staged.length} {t('main.git.staged')})</span>}
                        </span>
                        {gitStatus.conflictFileCount > 0 && (
                          <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13 }}>
                            ...
                            <span style={{ color: 'var(--red)' }}>{gitStatus.conflictFileCount} {t('main.git.conflicts')}</span>
                          </span>
                        )}
                      </div>
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        <button type="button" onClick={() => navigate('/git')} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 6, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', color: 'var(--text)', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>
                          <span className="codicon codicon-git-commit" style={{ fontSize: 13 }} aria-hidden />{t('main.git.commit')}
                        </button>
                        <button type="button" onClick={() => navigate('/git')} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 6, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', color: 'var(--text)', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>
                          <span className="codicon codicon-cloud-upload" style={{ fontSize: 13 }} aria-hidden />{t('main.git.push')}
                        </button>
                        <button type="button" onClick={() => navigate('/git')} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 6, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', color: 'var(--text)', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>
                          <span className="codicon codicon-cloud-download" style={{ fontSize: 13 }} aria-hidden />{t('main.git.pull')}
                        </button>
                      </div>
                    </>
                  ) : (
                    <p style={{ margin: 0, fontSize: 13, color: 'var(--text-muted)' }}>{t('main.git.notRepo')}</p>
                  )}
                </div>
              </div>
            )}
                      ...
                      <table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse' }}>
                        <thead>
                          <tr style={{ borderBottom: '1px solid var(--border)' }}>
                            <th style={{ padding: '8px 12px', textAlign: 'left', color: 'var(--text-muted)', fontWeight: 600 }}>{t('main.containers.name')}</th>
                            <th style={{ padding: '8px 12px', textAlign: 'left', color: 'var(--text-muted)', fontWeight: 600 }}>{t('main.containers.state')}</th>
                            <th style={{ padding: '8px 12px', textAlign: 'left', color: 'var(--text-muted)', fontWeight: 600 }}>{t('main.containers.image')}</th>
                          </tr>
                        </thead>
                        <tbody>
...
<div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                          ...
                          <div key={c.id} style={{ padding: '12px 14px', borderRadius: 8, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: portMatches.length > 0 ? 8 : 0 }}>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                ...
                                <button type="button" onClick={() => navigate('/docker')} title={t('main.services.viewDocker')} style={{ padding: '3px 8px', borderRadius: 4, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.04)', color: 'var(--text-muted)', cursor: 'pointer', fontSize: 11 }}>
                                ...
                                {portMatches.map((m, i) => {
                                  ...
                                  return (
                                    <span key={i} style={{ fontSize: 11, color: 'var(--accent)', background: 'rgba(var(--accent-rgb, 100,149,237), 0.1)', padding: '2px 7px', borderRadius: 4, fontFamily: 'monospace' }}>
                                      ...
                                      ) : t('main.ports.protocol', { protocol: m[3].toUpperCase(), port: hostPort })}
                                    </span>
                                  )
                                })}
                              </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 16, textAlign: 'center', color: 'var(--text-muted)' }}>
            ...
            <div>
              <p style={{ margin: '0 0 4px', fontSize: 17, fontWeight: 600, color: 'var(--text)' }}>{t('main.noProfiles.title')}</p>
      ...
      {confirmModalOpen && selectedProfile && (
        <div style={{
          ...
          <div style={{
            ...
            <h3 style={{ margin: '0 0 12px', fontSize: 18, fontWeight: 700 }}>{t('main.confirm.title')}</h3>
            <p style={{ margin: '0 0 20px', color: 'var(--text-muted)', fontSize: 14, lineHeight: 1.5 }}>
              {activeProfile ? (
                <>
                  {t('main.confirm.composeDown')} <strong style={{ color: 'var(--text)' }}>{activeProfile}</strong>, {t('main.confirm.thenStart')}{' '}
                  <strong style={{ color: 'var(--text)' }}>{selectedProfile.title}</strong>?
                </>
              ) : (
                <>{t('main.confirm.start')} <strong style={{ color: 'var(--text)' }}>{selectedProfile.title}</strong>?</>
              )}
            </p>
            <div style={{ display: 'flex', gap: 12 }}>
              <button
          ...
          <div className="fluent-modal-content" style={{ maxWidth: selectedProfile.name === 'data-science' ? 520 : 400 }}>
            {isScaffolding ? (
               <div style={{ textAlign: 'center', padding: '40px 0' }}>
                  ...
                  <p style={{ color: 'var(--text-muted)', marginBottom: 24 }}>{t('main.createProject.settingUp', { name: createProjectName })}</p>
                  ...
                  {installLogs.length > 0 && (
                    <div style={{ marginTop: 24, textAlign: 'left', background: 'rgba(0,0,0,0.4)', borderRadius: 8, border: '1px solid rgba(255,255,255,0.08)', overflow: 'hidden', boxShadow: '0 8px 32px rgba(0,0,0,0.2)' }}>
                      ...
                      <div style={{ background: 'rgba(255,255,255,0.03)', padding: '8px 12px', display: 'flex', alignItems: 'center', gap: 6, borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                        ...
                        <span style={{ marginLeft: 8, fontSize: 11, color: 'var(--text-muted)', fontFamily: 'sans-serif', letterSpacing: 0.5 }}>{t('main.createProject.installProgress')}</span>
                      </div>
                      {/* Terminal Body */}
              ...
              <>
                <h2 style={{ margin: '0 0 16px 0', fontSize: 24, fontWeight: 700 }}>{t('main.createProject.title')}</h2>
                <p style={{ margin: '0 0 24px', color: 'var(--text-muted)', fontSize: 15, lineHeight: 1.6 }}>
                  {t('main.createProject.description', { profile: selectedProfile.title, path: projectsHomeDir })}
                </p>
                <input
                  type="text"
                  autoFocus
                  ...
                  <div style={{ marginBottom: 20 }}>
                    ...
                    <div style={{ display: 'flex', gap: 10 }}>
                      {(['react-native', 'flutter'] as const).map((fw) => (
                        ...
                        >
                          {fw === 'react-native' ? t('main.createProject.reactNative') : t('main.createProject.flutter')}
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                {Object.keys(suggestedPorts).length > 0 && (
                  <div style={{ marginBottom: 20, padding: '12px 14px', borderRadius: 8, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
                    <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 0.8, textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 8 }}>{t('main.createProject.servicesPorts')}</div>
              ...
              <>
                 <h2 style={{ margin: '0 0 16px 0', fontSize: 24, fontWeight: 700 }}>{selectedProfileName === 'data-science' ? t('main.createProject.setupWizard', { profile: 'Data Science' }) : t('main.createProject.setupWizard', { profile: 'Web Development' })}</h2>
                 ...
                 {createProjectStep === 1 && (
                    <div style={{ animation: 'fade-in 0.3s ease' }}>
                      <p style={{ margin: '0 0 24px', color: 'var(--text-muted)', fontSize: 15, lineHeight: 1.6 }}>{t('main.createProject.step1')}</p>
                      
                      <strong style={{ display: 'block', marginBottom: 8, fontSize: 13, color: 'var(--text-muted)' }}>{t('main.createProject.projectName')}</strong>
                      <input
                        type="text"
                        autoFocus
                        value={createProjectName}
                        ...
                        placeholder={selectedProfileName === 'data-science' ? t('main.createProject.placeholderDataScience') : t('main.createProject.placeholderWebDev')}
                        style={{
                      ...
                      {selectedProfileName === 'data-science' ? (
                        <>
                          <strong style={{ display: 'block', marginBottom: 8, fontSize: 13, color: 'var(--text-muted)' }}>{t('main.createProject.pythonVersion')}</strong>
                          <select
                          ...
                          >
                            <option value="latest">{t('main.createProject.latestStableJupyter')}</option>
                        ...
                        <>
                          <strong style={{ display: 'block', marginBottom: 8, fontSize: 13, color: 'var(--text-muted)' }}>{t('main.createProject.nodeVersion')}</strong>
                          <select
                 ...
                 {createProjectStep === 2 && (
                    <div style={{ animation: 'fade-in 0.3s ease' }}>
                      <p style={{ margin: '0 0 24px', color: 'var(--text-muted)', fontSize: 15, lineHeight: 1.6 }}>{t('main.createProject.step2')}</p>
                      
                      <strong style={{ display: 'block', marginBottom: 8, fontSize: 13, color: 'var(--text-muted)' }}>{t('main.createProject.pgVersion')}</strong>
                      <select
                      ...
                      <div style={{ background: 'rgba(0,0,0,0.2)', padding: 16, borderRadius: 8, border: `1px solid ${selectedProfile.accent}50`, marginBottom: 32 }}>
                        <strong style={{ display: 'block', marginBottom: 8, color: selectedProfile.accent }}>{t('main.createProject.isolatedEnv')}</strong>
                        <p style={{ margin: 0, fontSize: 13, color: 'var(--text-muted)' }}>{t('main.createProject.isolatedDesc')}</p>
                      </div>
                    </div>
                 )}
                 {createProjectStep === 3 && (
                    <div style={{ animation: 'fade-in 0.3s ease' }}>
                      <p style={{ margin: '0 0 24px', color: 'var(--text-muted)', fontSize: 15, lineHeight: 1.6 }}>{t('main.createProject.step3')}</p>
                      
                      <div style={{ marginBottom: 20 }}>
                        <strong style={{ display: 'block', marginBottom: 12, fontSize: 12, textTransform: 'uppercase', letterSpacing: 1, color: 'var(--text-muted)' }}>{t('main.createProject.scaffoldFiles')}</strong>
                        {selectedProfileName === 'data-science' ? (
                          <>
                            <label style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12, cursor: 'pointer' }}>
                              ...
                              <span style={{ fontSize: 14 }}>{t('main.createProject.generateNotebook')}</span>
                            </label>
                            <label style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, cursor: 'pointer' }}>
                              ...
                              <span style={{ fontSize: 14 }}>{t('main.createProject.generateMainPy')}</span>
                            </label>
                          </>
                        ) : (
                          <>
                            <label style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12, cursor: 'pointer' }}>
                              ...
                              <span style={{ fontSize: 14 }}>{t('main.createProject.viteReact')}</span>
                            </label>
                          </>
                        )}
                      </div>
                      ...
                      <div style={{ marginBottom: 20 }}>
                        <strong style={{ display: 'block', marginBottom: 12, fontSize: 12, textTransform: 'uppercase', letterSpacing: 1, color: 'var(--text-muted)' }}>{selectedProfileName === 'data-science' ? t('main.createProject.corePythonLibs') : t('main.createProject.coreNPMLibs')}</strong>
                      ...
                      <div style={{ marginBottom: 8, paddingTop: 16, borderTop: '1px solid rgba(255,255,255,0.1)' }}>
                        <label style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
                          ...
                          <span style={{ fontWeight: 600, fontSize: 14, color: '#fff' }}>{t('main.createProject.autoInstall')}</span>
                        </label>
                        <p style={{ margin: '6px 0 0 28px', fontSize: 12, color: 'var(--text-muted)' }}>{t('main.createProject.autoInstallDesc', { lockfile: selectedProfileName === 'data-science' ? t('main.createProject.requirementsTxt') : t('main.createProject.packageJson') })}</p>
                      </div>
                    </div>
                 )}
