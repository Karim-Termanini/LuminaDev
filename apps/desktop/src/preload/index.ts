...
export type DhApi = {
  ...
  layoutGet: () => Promise<{ ok: boolean; layout: import('@linux-dev-home/shared').DashboardLayoutFile; error?: string }>
  ...
  jobStart: (payload: { kind: string; durationMs?: number; runtimeId?: string; version?: string; method?: 'system' | 'local'; removeMode?: 'runtime_only' | 'runtime_and_deps'; addToPath?: boolean }) => Promise<unknown>
  ...
  getHostPorts: () => Promise<{ ok: boolean; ports: import('@linux-dev-home/shared').HostPortRow[]; error?: string }>
  getHostSysInfo: () => Promise<{ ok: boolean; info: import('@linux-dev-home/shared').HostSysInfo; error?: string }>
...
const api: DhApi = {
  ...
  terminalClose: (id) => ipcRenderer.send(IPC.terminalClose, { id }),
  ...
  monitorTopProcesses: () => ipcRenderer.invoke(IPC.monitorTopProcesses),
  monitorSecurity: () => ipcRenderer.invoke(IPC.monitorSecurity),
  monitorSecurityDrilldown: () => ipcRenderer.invoke(IPC.monitorSecurityDrilldown),
  ...
  perfSnapshot: () => ipcRenderer.invoke(IPC.perfSnapshot),
