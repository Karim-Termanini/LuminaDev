...
| `/git-vcs` | partial | **Local repo:** `dh:git:vcs:*` status, stage/unstage, diff (text + binary detection), commit, **fetch** (remote dropdown from remote-tracking refs, default `origin`, `--prune`) + pull/push (HTTPS via Cloud Git tokens + `GIT_ASKPASS`), branch list (locals + **remotes** via `for-each-ref`), checkout/create, **dirty-checkout modal** + **`dh:git:vcs:stash`** for stash-and-switch. **Integrate:** `dh:git:vcs:merge` (optional `--ff-only`), `dh:git:vcs:rebase`, `dh:git:vcs:stash-pop`, `dh:git:vcs:merge-abort`, `dh:git:vcs:rebase-abort` (UI bar under sync). **Provider rail:** GitHub vs GitLab columns (`dh:cloud:auth:status` + `dh:git:vcs:remotes` URLs) with the same scoped host accent token family used on `/cloud-git`. **Not yet:** interactive conflict resolution / rebase --continue in-app (use editor + terminal for complex conflict flows). |
