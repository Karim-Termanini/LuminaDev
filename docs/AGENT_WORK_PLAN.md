...
| A4 | حدود الصلاحيات | مراجعة سريعة لمسارات Docker socket وSSH وأوامر shell: timeouts، allowlists، رسائل خطأ حتمية. **`جزئي` (2026-04-30):** `exec_output` / `exec_result` مع `tokio::time::timeout` (افتراضي 180s؛ sudo تحقق 30s؛ `git clone` 900s؛ خطوة `sudo` للتثبيت 900s) + بادئة `[HOST_COMMAND_TIMEOUT]` ورسائل واجهة؛ باقي allowlist/تدقيق أوامر `bash -lc` لاحقًا. |
