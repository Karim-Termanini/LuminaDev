...
## P8 — New Developer Dashboard Style Reference

### The Unified Mental Model
A developer working in LuminaDev needs to know 15 critical points of information, divided into 3 tiers of feedback frequency:

#### TIER 1: Critical Path (Check every 30 seconds)
1. **Git Status**: Branch name, uncommitted changes count, ahead/behind counters.
2. **Running Services**: Up/down flags, port mappings, health indicators, restart counts.
3. **Active Terminal Sessions**: Open shell instances, working directories, active command labels.
4. **Active Background Jobs**: Progress percentages, estimated completion time, cancellable flags.

#### TIER 2: Environment Health (Check every 5 minutes)
5. **Environment Variables**: Overridden vars, secret leaks warning, comparison with `.env.example`.
6. **Dependency Health**: Lockfile status, vulnerability scans (CVEs), outdated package alerts.
7. **Build Pipeline Status**: Linting/compile check errors, failed unit tests listings.
8. **Container Resource Usage**: Real-time CPU, memory, network and disk I/O metrics per container.
9. **Docker Storage Health**: Build cache size, container logs growth rate, volume pruning suggestions.
10. **Port Allocation**: Host:container forwarding maps, port conflict warnings.

#### TIER 3: Context & Collaboration (Check every 30 minutes)
11. **Team Activity**: Recent git commits, outstanding pull request review requests.
12. **Environment Drift**: Stale dependencies vs remote branch, changes in upstream compose files.
13. **Database Schema Status**: Pending database migrations, connection pool state.
14. **Framework-Specific Health**: HMR connection stability, framework compile alerts.
15. **Session Resume**: State restoring on launch (recently open files, active branches, previous logs).

### Technical Specifications
- **Git Status Panel**: Reads `GitStatusResponse` detailing staged/unstaged changes, current branch status.
- **Running Services Panel**: Maps Docker containers to host ports, polling `/docker` system states.
- **Container Resources**: Polled every 3 seconds from the backend stats stream.
- **Drift Detection**: Automated check against upstream configurations.
